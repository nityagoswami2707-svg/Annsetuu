      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 relative z-10">
        ...
        <div className="bg-gradient-to-r from-blue-900 via-emerald-950 to-orange-600 text-white rounded-3xl p-6 sm:p-10 shadow-xl border border-blue-800 text-center space-y-4">
          <div className="max-w-xl mx-auto space-y-1">
            ...
            <h1 className="text-2xl sm:text-4xl font-black font-outfit text-white">{t('trackFoodDonationHeader')}</h1>
            <p className="text-emerald-100 text-xs sm:text-sm">{t('trackHeaderSub')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white/95 backdrop-blur-md text-gray-900 rounded-3xl p-5 sm:p-7 shadow-md border border-gray-200/80 space-y-5">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  <div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{t('generatedDonationId')}</span>
                    <h3 className="text-xl font-black font-outfit text-emerald-950">{activeDonation.id}</h3>
                  </div>

                  <div className="text-right">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">{t('verificationStatusLabel')}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="bg-gray-50 p-3.5 rounded-2xl border border-gray-200">
                    <p className="text-[10px] text-gray-400 font-bold uppercase">{t('step1Title')}</p>
                  ...
                  <div className="bg-emerald-50/70 p-3.5 rounded-2xl border border-emerald-200">
                    <p className="text-[10px] text-emerald-800 font-bold uppercase">{t('ngos')}</p>
                ...
                {activeDonation.status === 'Delivered' && (
                  <div className="p-4 bg-amber-50 rounded-2xl border border-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="text-xs font-black text-amber-950">{t('step6Title')} ✓</h4>
                        <p className="text-[11px] text-amber-800">{t('step7Sub')}</p>
                      </div>
                    </div>
                    <button
            ...
            <div className="bg-white/95 backdrop-blur-md text-gray-900 rounded-3xl p-5 sm:p-7 shadow-md border border-gray-200/80 space-y-5">
              ...
              <div className="space-y-4 relative pl-6">
                ...
                {[
                  { status: "Donation Registered", label: t('step2Title'), icon: "✓" },
                  { status: "NGO Accepted", label: t('step4Title'), icon: "✓" },
                  { status: "Pickup Assigned", label: t('step5Title'), icon: "✓" },
                  { status: "Picked Up", label: t('step1PickedUp'), icon: "●" },
                  { status: "In Transit", label: t('step2InTransit'), icon: "●" },
                  { status: "Delivered", label: t('step6Title'), icon: "○" }
                ].map((stepItem, idx) => {
